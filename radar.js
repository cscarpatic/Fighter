...
// Scenario bounds are intentionally FIXED to historical/static objects.
// Player and fighters are excluded so their movement is visible on the map.
...
function plotContact(o,b,state){if(!alive(o))return;const{x,y}=map(o,b);if(!inMap(x,y))return;const type=o.userData?.type,target=shipTarget(o,state);if(type==='ship'||type==='carrier'){svg.appendChild(el('rect',{x:x-5,y:y-3,width:10,height:6,fill:'none',stroke:target?'#ff5252':'#63d8ff','stroke-width':target?2.5:1.7}));if(type==='carrier')svg.appendChild(el('line',{x1:x-7,y1:y,x2:x+7,y2:y,stroke:target?'#ff5252':'#63d8ff','stroke-width':1.5}));if(target){svg.appendChild(el('circle',{cx:x,cy:y,r:10,fill:'none',stroke:'#ff5252','stroke-width':2}));text(x+8,y-7,`TARGET ${o.userData?.name||''}`,'#ff9393',7)}}else{svg.appendChild(el('rect',{x:x-4,y:y-4,width:8,height:8,fill:o.userData?.objective?'#ffd84d':'#e6bf62',transform:`rotate(45 ${x} ${y})`}));if(o.userData?.objective)text(x+7,y-6,o.userData?.name||'','#ffe58a',7)}}
...
function draw(){requestAnimationFrame(draw);if(document.getElementById('game')?.classList.contains('hidden'))return;drawGrid();const state=window.AeroOpsState||{},player=state.player;if(!player?.position){text(105,98,'SINCRONIZZAZIONE…','#ffd88a',10,'middle');return}enhance(state);animateMarkers(performance.now()/1000);const b=bounds(state);for(const o of state.contacts||[])plotContact(o,b,state);for(const o of state.guns||[])plotAA(o,b);for(const o of state.enemies||[])plotEnemy(o,b);plotPlayer(player,b);text(205,184,zoom===1?'SCENARIO FISSO':'ZOOM','#a7cfdb',7,'end')}
