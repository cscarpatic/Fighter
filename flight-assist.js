...
const info=document.createElement('div');info.style.cssText='position:absolute;left:50%;bottom:16%;transform:translateX(-50%);padding:6px 10px;border-radius:7px;background:rgba(0,0,0,.48);color:#dff;font-size:12px;font-weight:800;letter-spacing:.04em';info.textContent='ASSISTENZA VOLO ATTIVA';
...
function projectPoint(world,camera){
  const q=world.clone().project(camera);
  if(q.z<-1||q.z>1)return null;
  return{x:(q.x*.5+.5)*innerWidth,y:(-.5*q.y+.5)*innerHeight};
}
function ballisticImpact(p){
  // Same gravity scale used by the bomb model, with forward speed approximating current arcade cruise.
  const g=24;
  const dir=new THREE.Vector3(0,0,-1).applyQuaternion(p.quaternion).normalize();
  const v=dir.multiplyScalar(82);
  const y0=Math.max(0,p.position.y),vy=v.y;
  const disc=vy*vy+2*g*y0;
  if(disc<0)return null;
  const t=(vy+Math.sqrt(disc))/g;
  if(!Number.isFinite(t)||t<=0||t>12)return null;
  return p.position.clone().addScaledVector(v,t).add(new THREE.Vector3(0,-.5*g*t*t,0));
}
...
function updateAim(p,state){
  ...
  if(target&&Math.abs(target.x)<.7&&Math.abs(target.y)<.55){
    const lateral=target.e.position.clone().sub(p.position),f=new THREE.Vector3(0,0,-1).applyQuaternion(p.quaternion),r=new THREE.Vector3(1,0,0).applyQuaternion(p.quaternion),u=new THREE.Vector3(0,1,0).applyQuaternion(p.quaternion),z=Math.max(1,f.dot(lateral)),x=r.dot(lateral)/z,y=u.dot(lateral)/z;
    gun.style.left=`${50+x*48}%`;gun.style.top=`${50-y*48}%`;gun.style.display='block';
  }else gun.style.display='none';

  const cam=cameraFor(p),impact=ballisticImpact(p),sp=cam&&impact?projectPoint(impact,cam):null;
  if(sp&&sp.x>-80&&sp.x<innerWidth+80&&sp.y>-80&&sp.y<innerHeight+80&&p.position.y>35){
    bomb.style.left=`${sp.x}px`;bomb.style.top=`${sp.y}px`;bomb.style.display='block';
...
function assist(now){
  ...
  const a=window.__AERO_ANALOG||{x:0,y:0,active:false};
